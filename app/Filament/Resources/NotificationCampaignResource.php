        ...
        return $form
            ->schema([
                ...
                Forms\Components\Section::make('Recipients')
                    ->schema([
                        Forms\Components\Toggle::make('send_to_all_faculty')
                        ...
                        Forms\Components\Select::make('recipient_ids')
                            ...
                            ->multiple()
                            ->searchable()
                            ->options(User::where('role', 'faculty')->pluck('name', 'id'))
                            ->hidden(fn (callable $get) => $get('send_to_all_faculty'))
                            ->columnSpanFull(),
                    ]),
        ...
        return $table
            ->columns([
                ...
                Tables\Columns\TextColumn::make('type')
                    ...
                    ->color(fn ($state) => match($state) {
                        NotificationType::SCHEDULE_CHANGE => 'warning',
                        NotificationType::CLASS_CANCELLATION => 'danger',
                        NotificationType::GRADE_SUBMISSION_REMINDER => 'info',
                        NotificationType::SYSTEM_MAINTENANCE => 'danger',
                        default => 'primary'
                    }),
            ...
            ->actions([
                Tables\Actions\Action::make('send')
                    ...
                    ->color('success')
                    ->visible(fn (NotificationCampaign $record) => $record->status === NotificationStatus::DRAFT || $record->status === NotificationStatus::SCHEDULED)
